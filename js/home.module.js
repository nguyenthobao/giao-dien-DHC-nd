...
function resetPoint(){
    ...
    sessionStorage.setItem('scale',scale);
        ...
        $.each(pointData, function (k, v) {
            ...
            if (v.point_images != '[]') html_marker += '<img style="transform:scale(1) !important;margin-top:0px" data-id="' + v.point_id + '" id="img_' + x + '"  ' + (pointImage[0] != undefined ? 'src="' + pointImage[0] + '"' : '') + ' class="img_instant img-fluid map" alt="" data-lat="' + v.lat + '" data-long="' + v.long + '">';
            html_marker += '</div>';
        });
        $('#content2 .content .row').append(html_marker);
        $('#choose').val($($('.img_instant')[0]).attr('style'));
        $('.img_instant').hide();
        $('.label_instant').hide();
        reset=0;
}