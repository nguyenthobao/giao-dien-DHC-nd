    ...
    // var scale=$('#mapdhc').css('transform')!='none'?parseFloat(($('#mapdhc').css('transform')).substring(7,14)):1;
     ...
     var scalehammer=$('#scalehammer').val()!=''?$('#scalehammer').val():1;
        ...
        $.each(pointData, function (k, v) {
            ...
            if (v.point_images != '[]') html_marker += '<img style="width:'+parseFloat(180/scalehammer)+'px !important;height:'+parseFloat(120/scalehammer)+'px !important;" data-id="' + v.point_id + '" id="img_' + x + '"  ' + (pointImage[0] != undefined ? 'src="' + pointImage[0] + '"' : '') + ' class="img_instant img-fluid map" alt="" data-lat="' + v.lat + '" data-long="' + v.long + '">';
            html_marker += '</div>';
        });
